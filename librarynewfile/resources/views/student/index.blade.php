                ...
                <a class="mt-4 bg-teal-200 text-black font-bold py-2 px-4 rounded" href="{{ route('add-student')}}">Add Student Infromation</a>
                    ...
                    <tbody>
                       ...
                       <tr>
                        ...
                        <td>
                            <a class="mt-4 bg-yellow-200 text-black font-bold py-2 px-4 rounded" href= "{{route('student-show', ['stuno' => $stuinfo->sno]) }}" >View</a>
                            <a class="mt-4 bg-pink-200 text-black font-bold py-2 px-4 rounded" href= "{{route('student-edit', ['stuno' => $stuinfo->sno]) }}" >Edit</a>
                            <form method="POST" action = "{{ route('student-delete', ['stuno' => $stuinfo->sno ])  }}" onclick="return confirm('Are you sure you want to delete this record?')">
                           @csrf
                           @method('delete')
